...
export function DocumentsDialog({ onClose }: { onClose: () => void }) {
  ...
  const onImport = async (picked: File) => {
    ...
    try {
      ...
      setError(
        err instanceof TransferError ? err.message : 'That file could not be opened.',
      )
    }
  }
  ...
  return (
    <Modal title="Documents" onClose={onClose} width={520}>
      ...
      {error && <p className="border border-[#FF8FA3] px-2 py-1.5 text-[11px] text-[#FF8FA3]">{error}</p>}

      <div className="border-t border-ink/25/15">
        {rows === null && <p className="py-3 text-xs text-dim">Loading…</p>}
        {rows?.length === 0 && (
          <p className="py-3 text-xs text-dim">Nothing saved yet.</p>
        )}
        {rows?.map((row) => {
