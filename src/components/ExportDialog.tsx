      ...
      for (const [i, lang] of langs.entries()) {
        ...
        const blob = await exportDeckPdf(
          ...
          assets,
          {
            ...
            onProgress: (done) => setProgress({ done: i * perLang + done, total: perLang * langs.length }),
          },
        )
    ...
    <Modal title="Export PDF" onClose={running ? () => abort.current?.abort() : onClose}>
      <div className="flex flex-col gap-2">
        ...
        <p className="text-2xs leading-snug text-dim">
          Text, rules and colour are vector at either setting — this only changes how finely
          photographs are embedded. Print doubles their resolution, and the file size with it.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <Segmented
          value={which}
          onChange={setWhich}
          options={[...LANGS.map((l) => ({ value: l as Which, label: LANG_LABEL[l] })), { value: 'both' as Which, label: 'Both' }]}
        />
        <p className="text-2xs leading-snug text-dim">
          One layout, one PDF per language. Anything without a translation is exported in the
          language it was written in.
        </p>
      </div>

      {error && <p className="rounded-2xl bg-[#FF8FA3]/10 px-3 py-2 text-2xs text-[#FF8FA3]">{error}</p>}
